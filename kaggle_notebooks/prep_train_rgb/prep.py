...
Chay tren Kaggle CPU (khong ton quota GPU). Xuat ra /kaggle/working/hodc_train:
...
OUT = "/kaggle/working/hodc_train_rgb"
# PHEP THU DOI CHUNG: chi giu 3 bang thay vi 16.
# Muc dich: do xem viec dung 16 kenh co THUC SU loi hon khong, khi tinh ca cai gia
# phai tra la mat trong so pretrain cua conv dau (3->16 phai "thoi phong").
# Chon 3 bang trai deu pho (dau / giua / cuoi) thay vi 3 bang lien ke nhu script mau.
BANDS = [0, 8, 15]
# Xuat uint8 theo dung cach script mau cua ban to chuc lam (min-max tung kenh, tung anh).
# Ly do: Ultralytics BAT cac augmentation danh cho anh mau khi so kenh == 3
# (CLAHE, HSV...) va chung deu gia dinh 8-bit. Giu uint16 se vo ngay o CLAHE.
# Quan trong hon: day moi la thu ma cac doi dung pseudo-RGB thuc su chay.
